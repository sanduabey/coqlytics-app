...
const getItemSellsByWallet = async (
  walletAddress: string,
  pageNumber: number
) => {
  const response = await fetch(
    `${process.env.HOST}/api/item/wallet-sells?walletAddress=${walletAddress}&pageNo=${pageNumber}`
  )
...
const RecentItemSellsByWalletList = (props: { walletAddress: string }) => {
  const {
    fetchNextPage,
    hasNextPage,
    isFetchingNextPage,
    ...
    ['itemsellsbywallet'],
    ({ pageParam = 0 }) => getItemSellsByWallet(props.walletAddress, pageParam),
    {
...
export default RecentItemSellsByWalletList
